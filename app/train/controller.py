...
class TrainController(ControllerBase):
    ...
    def __init__(self, model: ModelBase, view: ViewBase):
        ...
        self._set_tapping()
        self.view.set_slider(
            self.model.trig_level_min,
            self.model.trig_level_max,
            self.model.trig_level_val,
        )

        self.view.set_lcdNumberAll(
            self.model.tap_train_target_count,
    ...
    def on_pushButton_TapTrainSampleStart_clicked(self):
        ...
        self.view.lcdNumber_TapTrainSampleNumber.display(
            self.model.tap_train_sample_number
        )
    ...
    def _finish_rub_pretraining(self, frames):
        ...
        if buffer.size == 0:
            ...
            return
        if not self._validate_rub_capture(
            frames, self.model.rub_train_duration_sec, "Rub Pre-training"
        ):
            return
    ...
    def _finish_rub_training(self, frames):
        if not self.model.rub_pretrained:
            ...
            return
        if not self._validate_rub_capture(
            frames, self.model.rub_threshold_duration_sec, "Rub Training"
        ):
            return
        scores = self._compute_rub_scores(frames)
        if not scores:
        ...
        standardized = np.asarray(
            [self.model.standardize_pretrain(s) for s in scores], dtype=float
        )
    ...
    def _validate_rub_capture(self, frames, expected_duration, phase_name: str) -> bool:
        valid_frames = [
            frame for frame in frames if frame is not None and frame.size > 0
        ]
        if not valid_frames:
    ...
    def handle_train_data(self):
        if not self.model.trained:
            ...
            self.view.set_lcd(
                self.view.lcdNumber_TapTrainSampleNumber,
                self.model.tap_train_sample_number,
            )
    ...
    def handle_threshold_data(self):
        if not self.model.thresholded:
            ...
            self.view.set_lcd(
                self.view.lcdNumber_TapTHSampleNumber, self.model.tap_th_sample_number
            )
    ...
    def _delete_train_data(self):
        ...
        self.view.set_lcd(
            self.view.lcdNumber_TapTrainSampleNumber, self.model.tap_train_sample_number
        )
