...
class GMM(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        # === 学習データに対する logp の平均と分散を保存しておく ===
        logp_train = self.gmm_.score_samples(X)  # shape (n_frames,)
        self._logp_mean = float(logp_train.mean())
        self._logp_std = float(logp_train.std() + 1e-8)
        print(f"[GMM] train logp mean={self._logp_mean:.3f}, std={self._logp_std:.3f}")
    ...
    def _anomaly_scores(self, X):
        ...
        log_probs = self.gmm_.score_samples(X)   # (n_frames,)
        return -log_probs
